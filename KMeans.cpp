...
void KMeans::updateCore(IplImage *frame)
{		
	static int len=0;
	unsigned char *s,*v;
	int the_class;
	...
	else
	{
		...
		for(int i=0;i<frame->imageSize;i++)
		{
			...
			(*s) = (this->counter[the_class]*(*s)+(*v))/(this->counter[the_class]+1.0);
			s++;v++;
		}
...
//it returns -1
int KMeans::findClass(IplImage *frame)
{	
	int index=-1;
